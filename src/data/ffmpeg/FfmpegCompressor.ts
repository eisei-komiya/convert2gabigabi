...
/**
 * キャッシュディレクトリ内の古い一時出力ファイル（_compressed_, _gabigabi_, _converted_, _passlog を含むもの）を削除する。
 ...
 * 初回圧縮後もサイズが超過する場合は2パスエンコードでリトライする（最大3回）。
 *
...
async function compressVideoToTarget(
  ...
  for (let attempt = 0; attempt < 3 && outputBytes > targetBytes; attempt++) {
    ...
    const retryPasslogPath = `${cacheDir}${stem}_passlog_${retrySuffix}`;

    // 実際の出力サイズから再計算したビットレートでリトライ
    const retryTargetBits = targetBytes * 8 * 0.92;
    const retryBitrateKbps = Math.max(
      50,
      Math.floor(retryTargetBits / durationSec / 1000),
    );

    const retry1Cmd = [
      '-y',
      '-i', `"${inputPath}"`,
      '-b:v', `${retryBitrateKbps}k`,
      '-maxrate', `${retryBitrateKbps}k`,
      '-bufsize', `${retryBitrateKbps * 2}k`,
      '-pass', '1',
      '-passlogfile', `"${retryPasslogPath}"`,
      '-an',
      '-f', 'null',
      '/dev/null',
    ].join(' ');
    ...
    if (retryBytes > 0) {
      // (#192) 前回のリトライ出力ファイルを削除してから currentOutputUri を更新する
